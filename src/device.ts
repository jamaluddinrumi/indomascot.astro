...
function isMobileOrTablet(a: string): boolean {
  return (
    REGEX_MOBILE_OR_TABLET1.test(a) ||
    REGEX_MOBILE_OR_TABLET2.test(a.slice(0, 4))
  );
}
...
function generateFlags(headers, userAgent: string): Device {
  let mobile = false;
...
export default generateFlags({}, userAgent);
